...
def _cmd_joke(reply):
    ...
    if joke_en and any(ord(c) > 127 for c in joke_en[:20]):
        ...
        reply(call_gemini("說一個適合全家的台灣笑話"))
...
def _cmd_activity(reply):
    ...
    if data and data.get("activity"):
        ...
        reply(call_gemini("推薦一個適合全家一起做的休閒活動，用繁體中文回答"))
...
def _cmd_exercise(reply):
    ...
    if data:
        ...
        reply(call_gemini("推薦一個適合在家做的簡單運動，說明動作和次數"))
...
def _cmd_anime_quote(reply):
    ...
    if data and data.get("quote"):
        ...
        reply(call_gemini("給我一句著名動漫台詞，說出出自哪部作品"))
...
def _cmd_movie_quote(reply):
    ...
    if q and q.get("quote"):
        ...
        reply(call_gemini("給我一句著名電影台詞，說出電影名稱，用繁體中文"))
...
def _cmd_fun_fact(reply):
    ...
    if fact_en:
        ...
        reply(call_gemini("給我一個有趣的冷知識，用繁體中文"))
...
def _cmd_motivation(reply):
    ...
    if q and q.get("text"):
        ...
        reply(call_gemini("給我一句激勵人心的名言，用繁體中文"))
...
def _cmd_astronomy(reply):
    ...
    if fact_en:
        ...
        reply(call_gemini("給我一個有趣的天文或科學冷知識，用繁體中文"))
...
def _cmd_chuck(reply):
    ...
    if joke_en:
        ...
        reply(call_gemini("說一個關於超強壯男人的誇張笑話，用繁體中文"))
...
def _cmd_number_fact(reply):
    ...
    if fact_en:
        ...
        reply(call_gemini("給我一個關於數字的有趣冷知識，用繁體中文"))
...
def _cmd_jlpt_word(reply):
    ...
    if data:
        ...
        reply(call_gemini("給我一個 JLPT N5 等級的日文單字，包含：單字、假名讀音、繁體中文意思、一個例句"))
...
def _cmd_kanji_daily(reply):
    ...
    if data:
        ...
        reply(call_gemini(f"用繁體中文介紹日文漢字「{char}」，包含讀音、意思和一個例句"))
