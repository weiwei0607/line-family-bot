...
def handle_fun(reply_token: str, source, text: str) -> bool:
    ...
    if text in ["今天吃什麼", "吃什麼", "晚餐吃什麼", "午餐吃什麼"]:
        ...
        if meal:
            name_zh = call_gemini(f"翻成繁體中文，只給菜名翻譯：{meal['name']}")
            ingr = "、".join(meal["ingredients"][:6])
            ingr_zh = call_gemini(f"把這串食材翻成繁體中文，逗號分隔，只給翻譯：{ingr}")
    ...
    if text in ["笑話", "說個笑話", "講個笑話"]:
        joke_en = get_joke()
        if _q(joke_en, reply_token): return True
        if joke_en:
    ...
    if text in ["今日宇宙", "宇宙圖片", "NASA", "天文圖", "宇宙"]:
        ...
        if apod:
            title_zh = call_gemini(f"翻成繁體中文，只給標題翻譯：{apod['title']}")
    ...
    if m:
        ...
        if data:
            ...
            meanings_zh = call_gemini(
                f"把這些英文意思翻成繁體中文，逗號分隔，只給翻譯：{', '.join(data['meanings_en'])}"
            )
    ...
    if m:
        ...
        if data and data.get("definitions"):
            ...
            defs_zh = call_gemini(f"把以下西班牙文解釋翻成繁體中文，條列，只給翻譯：\n{defs}")
            reply(reply_token,
